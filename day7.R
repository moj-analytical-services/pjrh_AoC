...
input <- c("dull silver bags contain 2 striped magenta bags, 2 dark coral bags, 1 bright orange bag, 4 plaid blue bags.",
...
input_test <- c("light red bags contain 1 bright white bag, 2 muted yellow bags.",
...
# Part 1
colours_containing <- function(x, rules) {
  rules_select_ind <- str_detect(rules, x)
  rules_select <- rules[rules_select_ind]
  str_extract(rules_select, ".*(?= bags contain)") %>% setdiff(x)
}

reccursive_colours_containing <- function(new_bags, rules, tested_bags = list()) {
  
  loop_out_bags <- new_bags %>%
  map(~colours_containing(.x, rules)) %>%
  flatten()
  
  tested_bags = union(loop_out_bags, tested_bags)
  
  if (is_empty(loop_out_bags)) return(tested_bags)
  else return(reccursive_colours_containing(loop_out_bags, rules, tested_bags))
}

reccursive_colours_containing("shiny gold", input)
# The length of this list is the answer

# Part 2

colours_contained_by <- function(x, rules) {
  input_bag_colour <- str_remove(x, "[1-9*] ")
  input_bag_numbers <- str_extract(x, "[1-9*]+")
  
  rules_select_ind <- str_detect(rules, paste0("^",input_bag_colour))
  rules_select <- rules[rules_select_ind]
  how_many_inside <- str_extract_all(rules_select, "[1-9].*?(?= bag)")
  
  how_many_inside %>%
    map(~paste0(input_bag_numbers, "*", .x))
  }

reccursive_colours_contained_by <- function(new_bags, rules, finished_bags = list()) {
  
  loop_out_bags <- new_bags %>%
    map(~colours_contained_by(.x, rules)) %>%
    flatten() %>%
    flatten_chr()
  
  unfinished_bags = loop_out_bags[!str_detect(loop_out_bags, "\\*$")]
  finished_bags = c(finished_bags, unfinished_bags)

  if (is_empty(unfinished_bags)) return(finished_bags)
  else return(reccursive_colours_contained_by(unfinished_bags, rules, finished_bags))
}

bags_in_bags <- reccursive_colours_contained_by("1 shiny gold", input)

bags_in_bags %>%
  str_remove_all("[ a-z]") %>%
  paste0(collapse = "+")
# Then copy and paste the resulting string into console to evaluate...
