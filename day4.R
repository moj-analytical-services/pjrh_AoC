...
valid_part1 <- field_strings[field_strings %>% str_count(":") == 7]
length(valid_part1)

# Part 2

byr_regex <- seq(1920, 2002) %>% paste0(collapse = "|") %>% {paste0("byr:(", ., ")\\b")}
iyr_regex <- seq(2010, 2020) %>% paste0(collapse = "|") %>% {paste0("iyr:(", ., ")\\b")}
eyr_regex <- seq(2020, 2030) %>% paste0(collapse = "|") %>% {paste0("eyr:(", ., ")\\b")}

hgt_regex_cm <- seq(150, 193) %>% paste0("cm") %>% paste0(collapse = "|") %>% {paste0("hgt:(", ., ")")}
hgt_regex_in <- seq(59, 76) %>% paste0("in") %>% paste0(collapse = "|") %>% {paste0("hgt:(", ., ")")}
hgt_regex <- paste0("(", hgt_regex_cm, ")|(", hgt_regex_in, ")\\b")

hcl_regex <- "hcl:#[(0-9)(a-f)]{6}\\b"
ecl_regex <- "ecl:(amb|blu|brn|gry|grn|hzl|oth)\\b"
pid_regex <- "pid:[0-9]{9}\\b"

regexes <- list(byr_regex, iyr_regex, eyr_regex, hgt_regex, hcl_regex, ecl_regex, pid_regex)

valid_part2_ind <- regexes %>%
  map(~valid_part1 %>% str_detect(.x)) %>%
  reduce(`&`)

sum(valid_part2_ind)

valid_part1[valid_part2_ind]
