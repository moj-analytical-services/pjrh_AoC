input <- c("14-15 v: vdvvvvvsvvvvvfpv",
...
password_classify <- tibble(range_low = input %>% str_extract_all(., "[0-9]+") %>% map(~.x[1]) %>% as.numeric(),
                            range_high = input %>% str_extract_all(., "[0-9]+") %>% map(~.x[2]) %>% as.numeric(),
                            restricted_letter = input %>% str_extract_all(., "[A-z](?=:)"),
                            password = input %>% str_extract_all(., "[A-z]+$")) %>%
  unnest(cols = c(range_low, range_high, restricted_letter, password)) %>%
  mutate(target_letter_count = str_count(password, restricted_letter),
         is_valid_pt1 = (target_letter_count >= range_low) & (target_letter_count <= range_high),
         restricted_letter_loc = str_locate_all(password, restricted_letter)) %>%
  rowwise() %>%
  mutate(is_valid_pt2 = xor(range_low %in% restricted_letter_loc, range_high %in% restricted_letter_loc))

# Part 1
password_classify %>% pull(is_valid_pt1) %>% sum()

# Part 2
password_classify %>% pull(is_valid_pt2) %>% sum()
