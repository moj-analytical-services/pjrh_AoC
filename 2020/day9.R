...
# same approach as above, but this time use difference of cumulative sums
# to get all the possible sums contiguous ranges

input_temp1_part2 <- input %>%
  transmute(id1 = row_number(),
            val1 = value,
            cumval1 = cumsum(val1))

input_temp2_part2 <- input %>%
  transmute(id2 = row_number(),
            val2 = value,
            cumval2 = cumsum(val2))

input_xjoin_part2 <- crossing(input_temp1_part2, input_temp2_part2)

contiguous_sums <- input_xjoin_part2 %>%
  mutate(id1 = id1 + 1) %>% # because substracting cumsums leaves out the smallest value
  filter(id1 != id2) %>%
  mutate(diffs = cumval2 - cumval1)


range_ids <- contiguous_sums %>%
  filter(diffs == ans_part1) %>%
  select(id1, id2)
